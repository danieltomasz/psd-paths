...
def identify_and_handle_reference_channel(
    raw: mne.io.Raw, reference_names: List[str] = ["VREF", "Cz", "REF"]
) -> mne.io.Raw:
    """
    ...
    low_var_channels = [
        ...
        if raw.ch_names[i] not in raw.info["bads"]
    ]

    if low_var_channels:
        print(
            f"Found {len(low_var_channels)} channels with very low variance: {low_var_channels}"
        )
    ...
    if reference_found or low_var_channels:
        ...
        raw.info["bads"].extend(
            [ch for ch in channels_to_mark if ch not in raw.info["bads"]]
        )
        ...
        if reference_found and reference_found in raw.info["bads"]:
            ...
            if new_var > low_var_threshold:
                print(
                    f"Reference channel {reference_found} now has normal variance after re-referencing"
                )
...
def detect_bad_channels_comprehensive(
    ...
    if use_autoreject_epochs:
        ...
        try:
            ...
            bad_epochs_based = [
                ch
                for ch, rate in zip(epochs.ch_names, bad_rate_per_channel)
                if rate > epoch_threshold
            ]

            bad_channels_by_method["Epochs"] = bad_epochs_based
            print(
                f"   Epoch-based found {len(bad_epochs_based)} bad channels: {bad_epochs_based}"
            )
    ...
    all_bad_channels = list(
        set(ch for channels in bad_channels_by_method.values() for ch in channels)
    )
...
def detect_bad_segments_adaptive(
    ...
    events = mne.make_fixed_length_events(
        raw_marked, duration=epoch_length, overlap=overlap
    )
    ...
    print(
        f"Total bad time: {sum(bad_annotations.duration):.1f}s "
...
def create_analysis_epochs(raw: mne.io.Raw, length: float = 1.0, overlap: float = 0.5):
    ...
    events = mne.make_fixed_length_events(raw, duration=length, overlap=overlap)
