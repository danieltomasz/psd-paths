...
def find_project_root(
    start_path: Optional[Path] = None, marker_file: str = "settings.toml"
) -> Path:
    """
...
def load_config(config_path: Optional[Path] = None, force_reload: bool = False) -> Dict:
    """
    ...
    Args:
        config_path: Path to config file (default: searches for settings.toml)
        force_reload: If True, always reloads from disk (useful in notebooks)

    Note:
        In Jupyter notebooks, Python caches modules. If you modify settings.toml
        and don't see changes, either:
        1. Restart the kernel, or
        2. Use force_reload=True, or
        3. Reload the module with importlib.reload(spectral.utils)
    """
    ...
    # Always read from disk to get latest changes
...
class ProjectPaths:
    ...
    def _setup_paths(self):
        ...
        # BIDS data - where raw EEG files are stored in BIDS format
        # Check if bids_root is specified in config, otherwise default to data/bids
        bids_root = self.config["paths"].get("bids_root", "data/bids")
        self.data = self.root / bids_root / self.subject
        ...
        self.specparam = (
            self.outputs / "specparam" / self.subject
        )  # Spectral parameters
