...
Initially 38 subjects  were included into  this analysis (criteria for the availability of the cognitive test data was used). 
EEG  data were analysed using MNE Python (@gramfort2013MEGEEGData, version 1.10). 


_EEG were first physically referenced to Cz (VREF) during recording_
Data were recorded with the resolution 1000 Hz and  during the analysis resampled to 250 Hz and  notch filtered at 50 Hz to eleminate line power noise. 

_Bad fragments of data were identified using pyriemann package (riemanian potato) and annotated as bad._


A bandpass filter between 1 and 45 Hz was applied  using FIR filter. Bad channels were identified using the Python implementation of the Preprocessing Pipeline (PREP, @bigdely-shamlo2015PREPPipelineStandardized), were temporarily excluded from analysis and  later extrapolated after performing Independent Component Analysis (ICA).

The continuous data were segmented into 5s epochs with a 1.5s overlap between adjacent segments (representing a 30% overlap). This resulted in a 3.5s stride between epoch onsets.
...
with the settings `fit_params=dict(ortho=False, extended=True)`, which aligns with  extended Infomax algorithm [@ablin2018FasterIndependentComponent].
<!-- change into infoMAx -->
